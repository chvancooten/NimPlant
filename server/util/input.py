...
if os.name == "posix":
    import readline
    from .commands import getAllAvailableCommandsList
    commands = getAllAvailableCommandsList()
...
def promptUserForCommand():
    ...
    if os.name == "posix":
        ...
        command = session.prompt(f"NimPlant {np.id} $ > ", completer=nestedCompleter, complete_style=CompleteStyle.READLINE_LIKE, auto_suggest=AutoSuggestFromHistory())
