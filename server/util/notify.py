...
def notify_user(np):
    try:
        message = f"*A new NimPlant checked in!*\n\n```\nUsername: {np.username}\nHostname: {np.hostname}\nOS build: {np.osBuild}\nExternal IP: {np.ipAddrExt}\nInternal IP: {np.ipAddrInt}\n```"

        if os.getenv('TELEGRAM_CHAT_ID') is not None and os.getenv('TELEGRAM_BOT_TOKEN') is not None:
            # Telegram notification
            notify_telegram(message, os.getenv('TELEGRAM_CHAT_ID'), os.getenv('TELEGRAM_BOT_TOKEN'))
        else:
...
def notify_telegram(message, telegram_chat_id, telegram_bot_token):
    ...
    notification_request = 'https://api.telegram.org/bot' + telegram_bot_token + '/sendMessage?chat_id=' + telegram_chat_id + '&parse_mode=Markdown&text=' + message
